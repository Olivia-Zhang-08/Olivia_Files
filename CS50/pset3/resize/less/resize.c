// Olivia Zhang, P-Set 3, Resize (less)
// Resizes a BMP file
...
int main(int argc, char *argv[])
{
    ...
    // determine new padding for output scanlines
    ...
    {
        // iterate by scale times
        for (int j = 0; j < scale; j++)
        {
            // iterate over pixels in scanline
            for (int k = 0; k < imgwidthbefore; k++)
            {
                // buffer variable for rgbtriple
                RGBTRIPLE triple;
            ...
            // adjust file position cursor accordingly
            if (j < scale - 1)
            {
                fseek(inptr, - (3 * imgwidthbefore + paddingbefore), SEEK_CUR);
            }
